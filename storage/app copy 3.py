...
def build_figure(bcd, gcd, ocd):
    ...
    cell_opacity = 0.0 if mode not in ('Target Point', 'Gates') else 0.01
    cell_hover = 'Click to place target' if mode == 'Target Point' else ('Click to toggle gate' if mode == 'Gates' else None)
    fig.add_trace(go.Scatter(
        ...
        hoverinfo='skip' if mode not in ('Target Point', 'Gates') else None,
    ...
    for i, gate in enumerate(st.session_state.gates):
        ...
        fig.add_trace(go.Scatter(
            ...
            textfont=dict(size=36, color='#9400D3', family='Arial Black'),
            showlegend=False, hoverinfo='skip',
        ))
...
MODES = ['Starting Point', 'Target Point', 'Gates', 'Obstacles', 'Bottles']
...
def render_chart():
    ...
    if clicked:
        ...
        # Gates traces appear after cells, so gate markers shift cell_curve up per gate placed
        n_gates = len(st.session_state.gates)
        cell_curve = 1 + (1 if has_arrow else 0)
        obstacle_curve = cell_curve + 1 + n_gates
