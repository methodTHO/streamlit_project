...
def build_animation_figure(bcd, gcd, ocd):
    ...
    robot_trace = go.Scatter(
        ...
        mode='text', text=['🤖'],
        textposition='middle center',
        textfont=dict(size=48, color='black'),
...
with col_wp:
    ...
    if n_wp == 0:
        ...
        sp = st.session_state.start_point
        lines = []
        # Robot always enters from bottom facing north (+y) in rotated display coords
        ...
        for i, rp in enumerate(st.session_state.route_points):
            if sp:
                ...
                # Direction and distance of current segment
                seg_dx = rp[0] - prev_x
                seg_dy = rp[1] - prev_y
                ...
                wp_type = rp[2] if len(rp) > 2 else 'F'
                if wp_type == 'B':
                    # Backward move: robot doesn't turn, heading stays the same
                    turn_str = '0° S'
                    # do NOT update prev_rdx/rdy — heading is unchanged
                ...
                meta = f'{turn_str} / {dist_str}sq'
            else:
                coord = f'({rp[0]}, {rp[1]})'
                meta = ''
            wp_type = rp[2] if len(rp) > 2 else 'F'
            if wp_type == 'B':
                lines.append(f'{i + 1}. <b><span style="color:red">B</span></b> {coord} / {meta}')
            elif wp_type == 'G':
                lines.append(f'{i + 1}. <span style="color:#20B2AA">G</span> {coord} / {meta}')
            else:
                lines.append(f'{i + 1}. {coord} / {meta}')
