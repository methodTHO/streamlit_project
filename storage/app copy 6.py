...
def build_figure(bcd, gcd, ocd):
    ...
    if tp:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(symbol='x', size=20, color='rgba(204,0,0,0.35)',
                        line=dict(color='rgba(204,0,0,0.35)', width=3)),
            text=[target_label],
            textposition='top center',
            textfont=dict(size=13, color='rgba(204,0,0,0.35)', family='monospace'),
            showlegend=False, hoverinfo='skip',
        ))
...
with st.sidebar:
    ...
    if st.session_state.mode == 'Starting Point':
        ...
        st.write('Click any square to add a Move to Gate waypoint (offset 0.25 from center toward approach direction).')
            ...
            if mode == 'Move to Gate':
                ...
                if prev:
                    ...
                    if abs(ddx) >= abs(ddy) and ddx != 0:
                        ...
                        ndx, ndy = 0, 0
                    ox, oy = cd[0] - 0.25 * ndx, cd[1] - 0.25 * ndy
                else:
                    ox, oy = cd[0], cd[1]
    ...
    if n_wp == 0:
        ...
        for i, rp in enumerate(st.session_state.route_points):
            ...
            if wp_type == 'B':
                ...
                lines.append(f'{i + 1}. <span style="color:#20B2AA">G</span> {coord}')
            else:
